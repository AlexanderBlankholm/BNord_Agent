...
class FinalExcelComponentParser:
    """
    Final, refined parser for Excel files containing construction project components and pricing.
    Based on analysis of processed files and test results, this parser provides better
    category detection and pricing extraction.
    """
    ...
    def parse_excel_file(self, file_path: str) -> Optional[Dict[str, Any]]:
        ...
        Returns:
            Dictionary containing metadata and components, or None if parsing fails
        """
        try:
            # Read Excel file
    ...
    def _is_header_row(self, row: pd.Series) -> bool:
        ...
        for cell in row:
            if pd.notna(cell):
                ...
                if any(keyword in cell_str for keyword in ['tilbud', 'budget', 'dato', 'projekt', 'kategori', 'påslag', 'timer', 'takst', 'byggeselskabet']):
                    return True
        return False
    ...
    def _is_category_header(self, row: pd.Series) -> bool:
        ...
        for cell in row:
            if pd.notna(cell):
                cell_str = str(cell).lower()
                if any(keyword in cell_str for keyword in ['rum', 'værelse', 'badeværelse', 'køkken', 'gang', 'projektledelse', 'nedrivning', 'murer', 'tømrer', 'snedker', 'vvs', 'el', 'maler', 'gulve']):
                    return True
        return False
    
    def _is_summary_row(self, row: pd.Series) -> bool:
        """Check if a row is a summary or total row."""
        for cell in row:
            if pd.notna(cell):
                cell_str = str(cell).lower()
                if any(keyword in cell_str for keyword in ['pris ekskl', 'moms', 'pris inkl', 'total', 'sum', 'fradrag']):
                    return True
        return False
        ...
        for i, cell in enumerate(row):
            if pd.notna(cell):
                ...
                if cell_str and not cell_str.isdigit() and len(cell_str) > 3:
                    ...
                    if not any(keyword in cell_str.lower() for keyword in ['tilbud', 'budget', 'dato', 'projekt', 'kategori', 'rum', 'værelse', 'påslag', 'timer', 'takst', 'byggeselskabet', 'pris ekskl', 'moms', 'pris inkl', 'total', 'sum', 'fradrag']):
                        return cell_str
        return None
